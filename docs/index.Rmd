...
rest_dfm <- reviews %>% 
    ...
    dfm_remove(pattern = stopwords("english")) ## ignore common stopwords 
rest_dfm  
```
...
# Note that this dictionary includes compounds! (e.g., "best not", "better not")
```
...
news_dfm <- news_corp %>%
    tokens(remove_punct = TRUE, 
           remove_numbers = TRUE, 
           remove_symbols = TRUE) %>% 
    tokens_compound(data_dictionary_LSD2015) %>% # don't forget compound terms! you can use the compound terms that are pre-specified in a dictionary, such as the data_dictionary_LSD2015
    dfm(tolower = TRUE)  %>%
    dfm_remove(pattern = stopwords("english")) # now remove stopwords as a final step so "not_" has been captured
```

Then we apply the sentiment dictionary to count the number of positive and negative words and phrases in each article using the Lexicon Sentiment Dictionary (LSD). 
...
Let's review a couple of the articles to see if the sentiment measures appear accurate.
...
# Sum the total number of sentiment words (in this case columns 2-4 in our dataframe)
news_dict_df$total <- rowSums(news_dict_df[,c(2:4)])

```

Then we create a dataframe of the docvars to "left join" our sentiment counts to the metadata we require -- in this case, the section of the newspaper. We need this step in order to have the text, the docvars, and the sentiment values altogether.
...
Left join the two dataframes (meaning whichever item you start with goes on the left):

```{r, message=FALSE}

# Left_join docvars and the sentiment counts
...
Note there are other ways we could calculate a sentiment score, such as subtracting the total number of negative words from the total number of positive words (for a "net-sentiment" measure). Using a proportion or this kind of net-sentiment are two of the most common ways.

The filter steps below are to remove any newspaper sections that are not from the sections of core interest in this analysis.

```{r, message=FALSE}

# Filter result to news, editorial, and opinion comparison
news_analysis_section_comparison <- news_analysis[grep("\\bnews\\b|\\bopinion\\b|\\beditorial\\b", news_analysis$section_sum), ]
  # lost 1/3 obs

# Check balance of sections across sources
dplyr::count(news_analysis_section_comparison, Source)

# Get proportion of positive and negative by section (removing N/As)
by_section <- news_analysis_section_comparison %>%
  ...
  summarise(total_words = sum(total, na.rm = T),
    ...
    ungroup() %>% #now ungroup in order to not have problems later 
...
# Join (recall that our comparison version, using news, opinion, and editorial sections, is a subset of 674 obs)
news_analysis_section_comparison <- news_analysis_section_comparison  %>%
...
by_section_neutral <- news_analysis_section_comparison %>%
  group_by(section_sum) %>%
...
Note that we do not need to use `tokens_compound` because the dictionary does not include compound terms.
...
eu_toks <- tokens(korpus.euspeech, remove_punct=TRUE) %>%
                ...
                tokens_group(groups = country) # grouping by year
eu_dfm = dfm(eu_toks)
...
eu_poplib_analysis <- convert(dfm_eu_prop, "data.frame") %>% 
  ...
  filter(length >= 1200) # removing short speeches of less than 1200 words
...
1. Think back to the last module (word frequencies). What is the advantage of dictionary approaches over more simple word frequency approaches to answer questions about emotion and valence or other questions, such as whether Trump's tweets were written by different people on Android vs. iPhone?
...
2. Apply the NRC dictionary in Quanteda (use 'data_dictionary_NRC') to measure sentiment.

3. Find the average sentiment score for each year in the sample.
